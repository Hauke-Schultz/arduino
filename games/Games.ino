...
struct Pin {
  static const short joystickX = 0;   // joystick X axis pin
  static const short joystickY = 1;   // joystick Y axis pin
...
void selectGame() {
  int xpin = analogRead(Pin::joystickX);
  int ypin = analogRead(Pin::joystickY);
  ...
  if (xpin < 10) {
    ...
    gameSelect = (gameSelect <= 1) ? 1 : gameSelect - 1;
  } else if (ypin > 1000) {
    gameSelect = (gameSelect >= 3) ? 3 : gameSelect + 1;
  } else {
  }
  drawDigit(gameSelect);
  delay(100);
  if (button) {
    gameType = gameSelect;
    if (gameSelect == 1) {
...
void drawArrow(int d) {
  for (int col = 0; col < 8; col++) {
    for (int row = 0; row < 8; row++) {
      matrix.setLed(0, row, col, pgm_read_byte(&(arrows[d][row][col])));
    }
  }
}
...
void drawSmily(int d) {
  for (int col = 0; col < 8; col++) {
    for (int row = 0; row < 8; row++) {
      matrix.setLed(0, row, col, pgm_read_byte(&(smily[d][row][col])));
    }
  }
}
...
void showArrowMessage() {
  for (int d = 0; d < sizeof(arrowwMessage[0]) - 7; d++) {
    for (int col = 0; col < 8; col++) {
      delay(messageSpeed);
      for (int row = 0; row < 8; row++) {
        // this reads the byte from the PROGMEM and displays it on the screen
        matrix.setLed(0, row, col, pgm_read_byte(&(arrowwMessage[row][col + d])));
      }
    }
  }
}
...
void showSmilyMessage() {
  for (int d = 0; d < sizeof(smilyMessage[0]) - 7; d++) {
    for (int col = 0; col < 8; col++) {
      delay(messageSpeed);
      for (int row = 0; row < 8; row++) {
        // this reads the byte from the PROGMEM and displays it on the screen
        matrix.setLed(0, row, col, pgm_read_byte(&(smilyMessage[row][col + d])));
      }
    }
  }
}
...
void startSmilyGame() {
  ...
  if (xpin < 10) {
    ...
    smilyEye = (smilyEye > 12) ? 0 : smilyEye + 1;
  } else if (ypin > 1000) {
    smilyMouth = (smilyMouth > 12) ? 0 : smilyMouth + 1;
  } else if (button) {
    gameType = 0;
    delay(1000);
  } else {
  ...
  for (int col = 0; col < 8; col++) {
    for (int row = 0; row < 4; row++) {
      matrix.setLed(0, row, col, pgm_read_byte(&(eyes[smilyEye][row][col])));
    }
  }
  for (int col = 0; col < 8; col++) {
    for (int row = 4; row < 8; row++) {
      matrix.setLed(0, row, col, pgm_read_byte(&(mouths[smilyMouth][row][col])));
    }
  }
  //Serial.print(" smilyEye:"); Serial.print(smilyEye);
  //Serial.print(" smilyMouth:"); Serial.println(smilyMouth);
